...
const menuShortId = computed(() => route.params.menuShortId as string)
...
const { data: payload, error, pending } = await useAsyncData(
  `shop-menu-${identifier.value}-${menuShortId.value}`,
  () => $fetch<{ slug?: string, shop?: Shop, menu?: unknown, categories: MenuCategory[], items: MenuItem[], variants: MenuItemVariant[] }>(`/api/shop/${identifier.value}/menu/${menuShortId.value}`),
  { watch: [identifier, menuShortId], server: true }
)
...
if (redirectToSlug.value && payload.value && 'slug' in payload.value) {
  await navigateTo(`/${payload.value.slug}/menu/${menuShortId.value}?utm_source=qr-code`, { replace: true })
}
watch(redirectToSlug, (v) => {
  if (v && payload.value && 'slug' in payload.value) {
    navigateTo(`/${payload.value.slug}/menu/${menuShortId.value}?utm_source=qr-code`, { replace: true })
  }
}, { immediate: true })
    ...
    <template v-else>
      ...
      >
        ...
        <p class="text-center text-neutral-600">
          {{ (error as { statusMessage?: string })?.statusMessage === 'Menu not found' ? t('shop.noMenu') : (error as { statusMessage?: string })?.statusMessage === 'Shop not found' ? t('shop.notFound') : t('shop.error') }}
        </p>
      </div>
